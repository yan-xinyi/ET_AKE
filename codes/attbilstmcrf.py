...
class ATTBiLSTMCRF(nn.Module):
    def __init__(self, vocab_size, embed_dim,
                 hidden_dim, fs_num, num_layers=1, emb_dropout_value=0.3, lstm_dropout_value=0.3, linear_dropout_value=0.3, num_tags=6):  # emb_dropout_value=0.1, lstm_dropout_value=0.1, linear_dropout_value=0.1
        super(ATTBiLSTMCRF, self).__init__()
        ...
        self.tocrf = nn.Linear(4 * hidden_dim, num_tags)
...
def att_blcrf(train_path, test_path, vocab_path, logfile, save_path, feature, fs_num):
    ...
    model = ATTBiLSTMCRF(vocab_size=vocab_size,
                       embed_dim=embed_dim,
    ...
    for epoch in range(epochs):
        ...
        with tqdm(testLoader) as pbar_test:
            for inputs in pbar_test:
                _, outputs = model(inputs, feature, is_training=False)
